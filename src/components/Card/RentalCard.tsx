...
const RentalCard: React.FC<IRentalCardProps> = ({ setOpen , item , setPropertyId }) => {  
  console.log(item);
  return (
    <div className="flex lg:flex-row flex-col gap-6">
      <div className="lg:w-[344px] w-full lg:h-[226px] h-[150px] relative">
        <Image alt="Property" src={`${imageUrl}${item?.propertyId?.propertyImages[0]}`} fill />
      </div>

      <div className=" lg:px-0 px-3 pt-1">
        <h1 className="text-primary font-semibold text-[24px] leading-5">
          ${item?.propertyId?.price}<sub className="font-normal">/ {item?.propertyId?.priceType === "day"
                        ? `/pd`
        ...
        <div className="flex items-center gap-2">
          <Image
            alt="Logo"
            src={`${imageUrl}${item?.propertyId?.createdBy?.avatar}`}
            width={30}
            height={30}
            style={{ borderRadius: "100%", height:30 , width:30 , objectFit: "contain"  }}
          />
          <Heading
        ...
        <button
          onClick={() => {setOpen(true) , setPropertyId(item?.propertyId?._id)}}
