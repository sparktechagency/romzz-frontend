...
const Filter: React.FC<IFilterProps> = ({ open, setOpen, setFilter }) => {
  ...
  const filterData = JSON?.parse(localStorage.getItem("filter") as string);
          ...
          >
            <div className="col-span-12 flex items-center gap-4">
              <Radio.Group
                ...
                className="flex text-primary items-center justify-center rounded-xl"
              >
                {["Not Okay", "Okay", "Good", "Very Good  ", "Amazing"]?.map(
                  (option, index) => {
                    return (
